...
test.describe('Login Page Tests', () => {
    ...
    test ('PT: Valid Login', async ({page}) => {
        ...
        try {
            ...
            await page.getByPlaceholder(placeholders[0]).fill(credentials[0].lms_email_student);
            ...
            await page.waitForLoadState('load');
            await expect(page).toHaveURL("https://student.evsu-beta.centralizedinc.com/app/student");
            await page.waitForSelector('text=Logout');
            await page.screenshot({ path: 'screenshots/login/PT/student/Valid_Login.png' });
        }
        catch (error) {
    ...
    test ('NT: Empty Password', async ({page}) => {
        ...
        try {
            ...
            await page.getByPlaceholder(placeholders[0]).type(credentials[0].lms_email_student, { delay: 50 });
            await page.getByPlaceholder(placeholders[1]).click();
            await page.locator("text=Login").click();
            await expect(page.locator("form.space-y-4", { hasText: "required" })).toBeVisible();
            await page.screenshot({ path: 'screenshots/login/NT/student/Empty_password.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
    ...
    test ('NT: Empty Email', async ({page}) => {
        ...
        try {
            ...
            await page.locator("text=Login").click();
            await expect(page.locator("form.space-y-4", { hasText: "required" })).toBeVisible();
            await page.screenshot({ path: 'screenshots/login/NT/student/Empty_email.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
    ...
    test ('NT: Invalid Credential', async ({page}) => {
        ...
        try {
            ...
            await page.screenshot({ path: 'screenshots/login/NT/student/Invalid_Cred.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
    ...
    test ('NT: Invalid Email', async ({page}) => {
        ...
        try {
            ...
            await page.locator("text=Login").click();
            await expect(page.locator("text=Failed")).toBeVisible();
            await page.screenshot({ path: 'screenshots/login/NT/student/Invalid_Email.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
    ...
    test ('NT: Invalid Password', async ({page}) => {
        ...
        try {
            ...
            await page.locator("text=Login").click();
            await expect(page.locator("text=Failed")).toBeVisible();
            await page.screenshot({ path: 'screenshots/login/NT/student/Invalid_Password.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
    ...
    test ('PT: Remember Me', async ({page}) => {
        ...
        try {
            ...
            await expect(page.locator(`text=${credentials[0].lmsI_full_name}`)).toBeVisible();
            await page.screenshot({ path: 'screenshots/login/PT/student/Remember_me.png' });
        }
        catch (error) {
            console.error('Login failed:', error);
            throw error; // Re-throw to fail the test
